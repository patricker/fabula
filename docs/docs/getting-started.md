...
```rust
    // -- Define the "suspicious login" pattern --
...
```rust
    // -- Batch evaluation --
    let mut engine = SiftEngine::<MemGraph>::new();
...
```rust
    ...
    // Rebuild the pattern (the previous one was moved into the batch engine).
    let pattern = PatternBuilder::<String, MemValue>::new("suspicious_login")
        .stage("login_a", |s| s
            .edge("login_a", "type".into(), MemValue::Str("login".into()))
            .edge_bind("login_a", "user".into(), "user")
    ...
    let mut inc_engine = SiftEngine::<MemGraph>::new();
    ...
    let mut add_event = |graph: &mut MemGraph,
                          engine: &mut SiftEngine<MemGraph>,
                          id: &str, typ: &str, user: &str, extra_label: &str, extra_val: &str,
                          t: i64| {
